...
def draw_compass(surface, room, rooms):
    """Draw the finish-room direction arrow in the top-right corner.
    Called after _draw_fog() in main.py so it always renders on top of the fog."""
    ...
    if not finish_room or room.is_finish:                                        # skip if no finish room exists or we are already in it
        return
